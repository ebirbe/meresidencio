...
class Usuario_Controller extends Template_Controller
{
	...
	if ($this->_validar(TRUE))
	{
	    ...
	    if (isset($datos['confirmado'])
		)$usuario->confirmado = (boolean) $datos['confirmado'];
	    $usuario->nombre = $datos['nombre'];
	    $usuario->apellido = $datos['apellido'];
	    ...
	    if ($datos['fecha_nac'] == '')
		$usuario->fecha_nac = NULL;
	    else
		$usuario->fecha_nac = $datos['fecha_nac'];

	    //Condicionales si el usuario aun no ha seleccionado una region
	    if ($datos['estado'] == 0)
		$usuario->estado_id = NULL;
	    else
		$usuario->estado_id = $datos['estado'];
	    if ($datos['ciudad'] == 0)
		$usuario->ciudad_id = NULL;
	    else
		$usuario->ciudad_id = $datos['ciudad'];
	    if ($datos['zona'] == 0)
		$usuario->zona_id = NULL;
	    else
		$usuario->zona_id = $datos['zona'];
